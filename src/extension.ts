...
export function activate(context: vscode.ExtensionContext) {
    const exporter = vscode.commands.registerCommand('extension.exporter', async () => {
        const executeExportHelper = async () => {
            const osOption: string = osName();

            let extensions: string | null | undefined = await getExtensionList({
                osOption
            });
            ...
            if (commandOption === commandOptions[0]) {
                exportResult = await saveExtensionListAsFile({
                    commands: commandList.join('')
                });
                exportStatus = 'saved';
            } else if (commandOption === commandOptions[1]) {
                exportResult = await copyExtensionList({
                    osOption,
                    commands: commandList.join('')
                });
                exportStatus = 'copied';
        ...
        const executeDeleteHeloper = async () => {
        ...
        await executeExportHelper();
        await executeDeleteHeloper();
    });
}
